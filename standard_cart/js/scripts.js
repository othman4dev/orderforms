...
(function ($) {
  ...
  var _iCheck = "iCheck",
    ...
    _cursor = "cursor",
    _mobile =
      /ip(hone|od|ad)|android|blackberry|windows phone|opera mini|silk/i.test(
        navigator.userAgent
      ) ||
  ...
  function callbacks(input, checked, callback, keep) {
    if (!keep) {
      ...
      input[_callback]("change")
        [_callback]("ifChanged")
        [_callback]("if" + capitalize(callback));
    }
  }
  ...
  jQuery(document)
    .on("click", ".disable-on-click", function () {
      ...
      if (jQuery(this).hasClass("spinner-on-click")) {
        ...
        jQuery(icon).removeAttr("class").addClass("fas fa-spinner fa-spin");
      }
    })
...
function scrollToGatewayInputError() {
  var displayError = jQuery(
      ".gateway-errors,.assisted-cc-input-feedback"
    ).first(),
      ...
      switch (data.result) {
        ...
        case "linking_complete":
          ...
          if (data.remote_account.email) {
            ...
            accountInfo =
              data.remote_account.firstname +
              " " +
              data.remote_account.lastname;
          }

          accountInfo = accountInfo.trim();

          feedbackMsg = this.feedbackMessage("linking_complete")
            .trim()
        ...
        case "login_to_link":
          if (htmlTarget === targetLogin) {
            ...
            if (htmlTarget === targetRegister) {
              ...
              feedbackMsg = this.feedbackMessage(
              ...
              this.feedbackContainer().each(function (i, el) {
                ...
                container.fadeOut("slow", function () {
                  if (linkContext === "checkout-new") {
                    ...
                    feedbackMsg = self.feedbackMessage(
  ...
  registration: function () {
    this.prefillPassword = function (params) {
      ...
      if (typeof params.hideContainer === "undefined") {
        var id = jQuery("#inputSecurityQId").attr("id")
          ? "#containerPassword"
          : "#containerNewUserSecurity";
        params.hideContainer = jQuery(id);
        params.hideInputs = true;
      } else if (
        typeof params.hideContainer === "string" &&
        params.hideContainer.length
      ) {
      ...
      if (typeof params.form === "undefined") {
        params.form = {
          password: [{ id: "inputNewPassword1" }, { id: "inputNewPassword2" }],
        };
      }
  ...
  jqClient: function () {
    ...
    this.get = function (url, data, success, dataType) {
      return WHMCS.http.client.request(
        jQuery.extend(_getSettings(url, data, success, dataType), {
          type: "GET",
        })
      );
    };
  ...
  dataTable: function () {
      ...
      if (typeof self.tables[id] === "undefined") {
        if (typeof options === "undefined") {
          options = {
            ...
            language: {
              emptyTable: el.data("langEmptyTable")
                ? el.data("langEmptyTable")
                : "No records found",
            },
          };
        }
    ...
    this.rebindConfirmation = function (e) {
      ...
      for (var i = 0, len = toggles.length; i < len; i++) {
        jQuery(toggles[i]).on("confirmed.bs.confirmation", function (e) {
          e.preventDefault();
          WHMCS.http.jqClient
  ...
  jsonForm: function () {
    ...
    this.markFieldErrors = function (form, fields) {
      ...
      for (var fieldName in fields) {
        ...
        fieldLookup = self.managedElements
          .split(",")
    ...
    this.onError = function (form, jqXHR) {
      if (
        ...
        typeof jqXHR.responseJSON.fields === "object"
      ) {
  ...
  effects: function () {
    this.errorShake = function (element) {
      ...
      return element.each(function () {
        ...
        animation1[ref] =
          (motion === "pos" ? "+=" : "-=") + options.distance * 2;
        animation2[ref] =
          (motion === "pos" ? "-=" : "+=") + options.distance * 2;

        el.animate(animation, options.speed);
        for (var i = 1; i < options.times; i++) {
          el.animate(animation1, options.speed).animate(
            animation2,
            options.speed
          );
        }
  ...
  this.register = function () {
    ...
    recaptchaForms.each(function (i, el) {
      ...
      var frm = jQuery(el),
        ...
        required =
          typeof requiredText !== "undefined" ? requiredText : "Required",
      ...
      if (recaptchaElement.length) {
        recaptchaElement.attr(
          "id",
          recaptchaElement.attr("id") + recaptchaCount
        );
    ...
    window.recaptchaLoadCallback = function () {
      jQuery(".g-recaptcha").each(function (i, el) {
        ...
        var recaptchaId = grecaptcha.render(el, {
          ...
          size: btn.hasClass("btn-recaptcha-invisible")
            ? "invisible"
            : "normal",
...
(function (module) {
  ...
  this.simpleRNG = function () {
    ...
    for (var x = 0; x < r; x++) {
      ...
      if (Math.random() > 0.5) {
        s += chars.substr(Math.floor(Math.random() * 13), 1);
      }
    }
  ...
  this.validateBaseUrl = function () {
    if (typeof window.whmcsBaseUrl === "undefined") {
      console.log(
        "Warning: The WHMCS Base URL definition is missing " +
  ...
  jQuery("#main-body")
    ...
    .on("click", ".order-button, .order-btn, .btn-order-now", function (e) {
      if (jQuery(this).data("hasRecommendations") == 1) {
        ...
        displayRecommendations(href, "addproductajax=1", true).done(
          function () {
...
function setRecommendationColors() {
  ...
  jQuery(".product-recommendations .product-recommendation").each(function () {
    ...
    if (
      ...
      primaryColor.match(/^#[0-9A-Fa-f]{3,6}$/gi) == undefined
    ) {
...
jQuery(document).ready(function () {
  ...
  function repositionScrollingSidebar() {
    ...
    var offsetTop = 0;
    var productListing = jQuery("#scrollingPanelContainer").prev(
  ...
  jQuery("#frmConfigureProduct").submit(function (e) {
    ...
    jQuery(button)
      ...
      .removeClass("fa-arrow-circle-right")
      .addClass("fa-spinner fa-spin");
    displayRecommendations(postUrl, "addproductajax=1&" + postData, false).done(
      function () {
  ...
  jQuery("#frmProductDomain").submit(function (e) {
    ...
    suggestions.find("div:not(.actions)").hide().end().find(".clone").remove();
          ...
          jQuery.each(data.result, function (index, domain) {
            ...
            if (
              ...
              if (domain.isAvailable && typeof pricing !== "string") {
                if (
                  domain.domainName !== domain.idnDomainName &&
                  idnLanguage.not(":visible")
                ) {
                ...
                if (domain.preferredTLDNotAvailable) {
                  unavailableTld
                    .show()
                    .find("strong")
                    .html(domain.originalUnavailableTld);
                }
                contactSupport.hide();
      ...
      suggestion
        .done(function (data) {
          ...
          jQuery.each(data.result, function (index, domain) {
            ...
            var newSuggestion = suggestions
              .find("div.domain-suggestion")
              .last();
            newSuggestion
              .addClass("clone")
              .find("span.domain")
            ...
            furtherSuggestions = suggestions
          ...
          jQuery.each(data.result, function (index, result) {
            if (result.status === true) {
              displayRecommendations(
                ...
                window.location =
                  whmcsBaseUrl + "/cart.php?a=confproduct&i=" + result.num;
              });
            } else {
              jQuery(".domain-lookup-primary-loader").hide();
  ...
  jQuery(".payment-methods").on("ifChecked", function (event) {
    ...
    if (jQuery(this).hasClass("is-credit-card")) {
      ...
      existingCards.each(function (index) {
        ...
        var payMethodElements = jQuery(
          '[data-paymethod-id="' + payMethodId + '"]'
        );
  ...
  jQuery(".cc-input-container .paymethod-info").click(function () {
    ...
    var input = jQuery(
      'input[name="ccinfo"][value=' + payMethodId + "]:not(:disabled)"
    );
    ...
    suggestion
      .done(function (data) {
        ...
        jQuery.each(data.result, function (index, domain) {
          ...
          suggestionCount++;
          if (domain.group) {
            newSuggestion.find("span.promo").addClass(domain.group).show();
            newSuggestion.find("span.sales-group-" + domain.group).show();
          }
  ...
  jQuery(".btn-add-to-cart").on("click", function () {
    ...
    var addToCart = WHMCS.http.jqClient
      .post(
        whmcsBaseUrl + "/cart.php",
        {
          a: "addToCart",
  ...
  jQuery("#frmDomainTransfer").submit(function (e) {
    ...
    WHMCS.http.jqClient
      .post(frmDomain.attr("action"), frmDomain.serialize(), null, "json")
      .done(function (data) {
  ...
  useCreditOnCheckout.on("ifChecked", function () {
    ...
    if (radio.prop("checked")) {
      ...
      if (isCcSelected && firstNonCcGateway.length !== 0) {
        ...
        (!shouldHideContainer || isCcSelected) &&
  ...
  jQuery("#domainRenewalFilter").on("keyup", function () {
    ...
    jQuery("#domainRenewals")
      ...
      .filter(function () {
        jQuery(this).toggle(
          jQuery(this).data("domain").toLowerCase().indexOf(inputText) > -1
        );
      });
  });
  ...
  jQuery(".payment-methods:checked").trigger("ifChecked");
  if (
    existingCardContainer.is(":visible") &&
    existingCardContainer.find("input.existing-card").length > 0
  ) {
    newCardInfo.slideUp();
  }
});
...
function validateCheckoutCreditCardInput(e) {
  var newOrExisting = jQuery('input[name="ccinfo"]:checked').val(),
    ...
    cardType = null,
    submit = true,
    selectedPaymentMethod = checkoutForm.find(
      'input[name="paymentmethod"]:checked'
    ),
    isCreditCardGateway = selectedPaymentMethod.hasClass("is-credit-card"),
    isRemoteCard =
      selectedPaymentMethod.data("payment-type") === "RemoteCreditCard",
  ...
  if (isCreditCardGateway && !isRemoteCard) {
    ...
    if (newOrExisting === "new") {
      ...
      cardType = jQuery.payment.cardType(
    ...
    if (
      cvvField.is(":visible") &&
      !jQuery.payment.validateCardCVC(cvvField.val(), cardType)
    ) {
...
function selectDomainPeriodInCart(domainName, price, period, yearsString) {
  ...
  var update = WHMCS.http.jqClient.post(window.location.pathname, {
    domain: domainName,
    period: period,
  ...
  update.done(function (data) {
    ...
    data.domains.forEach(function (domain) {
      jQuery("[name='" + domain.domain + "Price']")
        .parent("div")
        .find(".renewal-price")
        .html(domain.prefixedRenewPrice + domain.shortRenewalYearsLanguage);
    });
